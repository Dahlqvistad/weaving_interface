...
export default function InfoCard({ machineId, className = '' }: InfoCardProps) {
    ...
    const handleNameSubmit = async () => {
        ...
        setIsUpdating(true);
        try {
            const response = await fetch(`http://192.168.88.118:8080/api/machines/${machine.ip}/name`, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                ...
                setMachine(prev => prev ? { ...prev, name: editName.trim() } : null);
                setIsEditing(false);
            } else {
                ...
                <div className="py-1 px-2 bg-vs-gray-500 rounded-md mb-1 flex flex-row justify-center items-center gap-3">
                    {isEditing ? (
                        <input
                            ...
                            autoFocus
                            disabled={isUpdating}
                            style={{ width: `${Math.max(editName.length * 8, 80)}px` }}
                        />
                    ) : (
                    ...
                    <div 
                        className={`cursor-pointer hover:opacity-80 ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`}
